...
function readProgress(): number[] {
  try {
    ...
    return Array.isArray(parsed) ? parsed.filter(value => Number.isInteger(value) && value >= 0 && value < steps.length) : [];
  } catch {
    return [];
  }
}
...
export default function ScannerTour() {
  ...
  useEffect(() => { setCompleted(readProgress()); setReady(true); }, []);
  ...
  const reset = () => { setCompleted([]); window.localStorage.removeItem(storageKey); };
